...
{   
	...
	{  
		traverseANDprint(root); //Real printing function
	}

}

bool StudentList::traverseANDprint(StudentNode* root) 
{   
	 StudentNode* node = root;
	 bool done=false;
  stack<StudentNode*> stack1;// For the general structure of a BTree, staticly allocated stack is hardly to overflow.
  while (!done){             // But it is true when tree is optimized.
  ...
  return done;
	...
	//if(root->left !=0) traverseANDprint(root->left);
	//cout << *root << endl;
	//if(root->right!=0) traverseANDprint(root->right);
	//return true;
}

bool StudentList::search(int temp)  //search algorithm used in Part B
{   
	return traverseANDsearch(root,temp);  
}

bool StudentList::traverseANDsearch(StudentNode* root, int temp)
{   
	if(root==0 || root->numiD < 0 || root->numiD >9999999999)
	{ 
...
bool StudentList::removefromlist(int given) //Recursive deletion Method
{	
	if(size==0)
	{
	...
	else
	{
		...
		{
			Delete(root,given); //Real delete function ,see Dale's book
...
bool StudentList::Delete(StudentNode* &root,int given)
...
void StudentList::DeleteNode(StudentNode* &root)
{   
...
void StudentList::GetPredecessor(StudentNode* root,StudentNode& data) //Deleting Mechanism, see Dale's book
{
